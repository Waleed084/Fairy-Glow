...
const columns = [
  ...
  { id: 'siplet', label: 'Split Base', minWidth: 100 },
...
const TDevanHeadTable = ({ selectedOrders, thisMonthSubmissions, onCheckboxChange, checkboxState }) => {
  ...
  useEffect(() => {
    const fetchThisMonthSubmissions = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_HOST}/api/submissions/devan/${username}`);
